...
public class Algorithms extends PathSearching {
  //DIJKSTRA WORKS BY PROPAGATING OUTWARDS UNTIL IT FINDS THE FINISH AND THEN WORKING ITS WAY BACK TO GET THE PATH
		...
		public void Dijkstra() {
			...
			while(solving) {
				...
				final ArrayList<Node> explored = exploreNeighbors(priority.get(0), hops);	//CREATE AN ARRAYLIST OF NODES THAT WERE EXPLORED
				if(explored.size() > 0) {
  ...
  //A STAR WORKS ESSENTIALLY THE SAME AS DIJKSTRA CREATING A PRIORITY QUE AND PROPAGATING OUTWARDS UNTIL IT FINDS THE END
		...
		public void AStar() {
			...
			while(solving) {
				...
				final ArrayList<Node> explored = exploreNeighbors(priority.get(0),hops);
				if(explored.size() > 0) {
					priority.remove(0);
		...
		public ArrayList<Node> exploreNeighbors(final Node current, final int hops) {	//EXPLORE NEIGHBORS
			...
			for(int a = -1; a <= 1; a++) {
				for(int b = -1; b <= 1; b++) {
					...
					if((xbound > -1 && xbound < cells) && (ybound > -1 && ybound < cells)) {	//MAKES SURE THE NODE IS NOT OUTSIDE THE GRID
						final Node neighbor = map[xbound][ybound];
		...
		public void explore(final Node current, final int lastx, final int lasty, final int hops) {	//EXPLORE A NODE
